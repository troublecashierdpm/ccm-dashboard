  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9] font-sans flex text-gray-800">
      <aside className={`fixed inset-y-0 left-0 w-64 bg-white shadow-2xl z-50 transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="bg-[#C80082] p-5 text-white flex items-center justify-between"><div className="flex items-center gap-2 font-black text-sm tracking-wider"><span className="bg-white text-[#C80082] px-2 py-1 rounded font-black">AEON</span> TRC PANEL</div><button onClick={() => setSidebarOpen(false)} className="md:hidden text-white font-bold text-xl">✕</button></div>
      ...
      <main className="flex-1 md:ml-64 min-w-0 p-6">
        <header className="flex items-center justify-between bg-white px-6 py-4 rounded-2xl shadow-sm border mb-6">
          <div className="flex items-center gap-4"><button onClick={() => setSidebarOpen(true)} className="md:hidden text-2xl">☰</button><h2 className="font-black text-lg text-gray-800 uppercase tracking-wide">{selectedKaryawan ? `Profil: ${selectedKaryawan.nama}` : activePanel === "dir" ? "Direktori Karyawan DPM" : `Panel ${activePanel}`}</h2></div>
          {selectedKaryawan && <button onClick={() => setActivePanel("dir")} className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-xl font-bold text-xs uppercase transition">← Kembali</button>}
        </header>

        {loading ? ( <div className="flex flex-col items-center justify-center py-40 gap-3"><div className="w-12 h-12 border-4 border-[#C80082] border-t-transparent rounded-full animate-spin"></div><p className="text-gray-400 font-bold text-xs tracking-widest uppercase animate-pulse">Memuat Database Supabase...</p></div> ) : (
          <>
            {activePanel === "dir" && !selectedKaryawan && (
              <div className="space-y-10">
                {Object.entries(getGroupedKaryawan()).map(([under, members]) => (
                  <div key={under} className="bg-white p-6 rounded-3xl border shadow-sm"><h3 className="font-black text-sm text-gray-400 uppercase tracking-widest border-b pb-3 mb-6 flex items-center gap-2"><span className="w-2.5 h-2.5 bg-pink-500 rounded-full"></span> Under Leader: <span className="text-[#C80082] font-black">{under}</span> ({members.length} Staff)</h3><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      {members.map(k => (
                        <div key={k.nik} onClick={() => handleKaryawanClick(k)} className="bg-gray-50/50 hover:bg-white p-4 rounded-2xl border border-gray-100 shadow-sm text-center cursor-pointer transition-all duration-300 hover:-translate-y-1.5 hover:shadow-md hover:border-pink-200 group active:scale-95"><img src={getPhotoUrl(k.file_id, k.nama)} onError={(e) => { e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(k.nama)}&background=FCE7F3&color=E20074&bold=true`; }} referrerPolicy="no-referrer" className="w-20 h-24 object-cover rounded-xl mx-auto border-2 border-gray-200 group-hover:border-[#C80082] shadow-sm transition-colors" alt={k.nama} /><h4 className="font-extrabold text-xs text-gray-800 mt-3 leading-snug break-words max-w-full px-1 group-hover:text-[#C80082] transition-colors">{k.nama}</h4><p className="text-[10px] text-gray-400 font-semibold mt-1">NIK: {k.nik}</p></div>
                      ))}
                    </div></div>
                ))}
              </div>
            ...
            {["shortage", "ecobag", "member", "sakit", "sp"].includes(activePanel) && !selectedKaryawan && (
              <div className="space-y-6">
                <div className="bg-white p-5 rounded-2xl border shadow-sm flex flex-wrap items-end gap-4">
                  <div className="flex flex-col gap-1.5 min-w-[140px] flex-1 sm:flex-none"><label className="text-[9px] font-black tracking-wider uppercase text-gray-400">Cari Nama Karyawan</label><input type="text" placeholder="Ketik nama..." value={searchNama} onChange={(e) => setSearchNama(e.target.value)} className="p-3 border rounded-xl bg-gray-50 outline-none text-xs font-bold focus:ring-2 focus:ring-pink-400" /></div>
                  <div className="flex flex-col gap-1.5 min-w-[120px]"><label className="text-[9px] font-black tracking-wider uppercase text-gray-400">Filter Bulan / Periode</label><input type="text" placeholder="Contoh: 2025-10" value={filterBulan} onChange={(e) => setFilterBulan(e.target.value)} className="p-3 border rounded-xl bg-gray-50 outline-none text-xs font-bold focus:ring-2 focus:ring-pink-400" /></div>
                  {["sakit", "sp"].includes(activePanel) && ( <div className="flex flex-col gap-1.5 min-w-[140px]"><label className="text-[9px] font-black tracking-wider uppercase text-gray-400">Filter Keterangan/Surat</label><input type="text" placeholder="Sakit, SP1, BA..." value={filterTipe} onChange={(e) => setFilterTipe(e.target.value)} className="p-3 border rounded-xl bg-gray-50 outline-none text-xs font-bold focus:ring-2 focus:ring-pink-400" /></div> )}
                  {activePanel === "ecobag" && ( <div className="flex items-center gap-3 p-3 bg-gray-50 border rounded-xl text-xs font-bold text-gray-600"><label className="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" checked={showL} onChange={(e)=>setShowL(e.target.checked)} /> LA</label><label className="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" checked={showM} onChange={(e)=>setShowM(e.target.checked)} /> ME</label><label className="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" checked={showS} onChange={(e)=>setShowS(e.target.checked)} /> SM</label></div> )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="bg-white p-5 rounded-2xl border border-b-4 border-b-pink-500 shadow-sm"><p className="text-[9px] font-black text-gray-400 uppercase tracking-wider">Total Baris Terfilter</p><h3 className="text-xl font-black mt-1">{filteredData.length} baris</h3></div>
                  {activePanel === "shortage" && ( <><div className="bg-white p-5 rounded-2xl border border-b-4 border-b-red-500 shadow-sm"><p className="text-[9px] font-black text-gray-400 uppercase tracking-wider">Total Akumulasi Short</p><h3 className="text-xl font-black text-red-600 mt-1">Rp {gSum.card1.toLocaleString("id-ID")}</h3></div><div className="bg-white p-5 rounded-2xl border border-b-4 border-b-green-500 shadow-sm"><p className="text-[9px] font-black text-gray-400 uppercase tracking-wider">Total Akumulasi Over</p><h3 className="text-xl font-black text-green-600 mt-1">Rp {gSum.card2.toLocaleString("id-ID")}</h3></div></> )}
                  {(activePanel === "ecobag" || activePanel === "member") && ( <div className="bg-white p-5 rounded-2xl border border-b-4 border-b-[#C80082] shadow-sm"><p className="text-[9px] font-black text-gray-400 uppercase tracking-wider">Total Kantong/Member Terjual</p><h3 className="text-xl font-black text-[#C80082] mt-1">{gSum.card1.toLocaleString("id-ID")} Pcs</h3></div> )}
                </div>

                <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
                  <div className="p-4 bg-gray-50 border-b flex justify-between items-center text-xs font-bold text-gray-500"><span>Menampilkan {filteredData.length} Data Real Supabase</span></div>
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap gap-2 bg-white p-3 rounded-2xl border shadow-sm">
                  {[{ id: "shortage", label: "📊 Shortage", count: empStats.shortage }, { id: "ecobag", label: "🛍️ Ecobag", count: empStats.ecobag }, { id: "member", label: "💳 Member", count: empStats.member }, { id: "sp", label: "📄 SP/BA", count: empStats.sp }, { id: "sakit", label: "🏥 Sakit", count: empStats.sakit }].map(tab => (
                    <button key={tab.id} onClick={() => setEmpMenu(tab.id)} className={`px-4 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all flex items-center gap-2 ${empMenu === tab.id ? "bg-[#C80082] text-white shadow-md" : "bg-gray-50 text-gray-500 hover:bg-pink-50"}`}>{tab.label} <span className={`text-[10px] px-2 py-0.5 rounded-full ${empMenu === tab.id ? "bg-white/20 text-white" : "bg-gray-200 text-gray-600"}`}>{tab.count}</span></button>
                  ))}
                </div>

                {["shortage", "member", "ecobag"].includes(empMenu) && ( <div className="bg-white p-6 rounded-3xl border shadow-sm"><div className="h-64 relative w-full"><canvas id="individualChartCanvas"></canvas></div></div> )}
                  ...
                  <div className="overflow-x-auto max-h-[50vh] overflow-y-auto">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y text-gray-700 font-medium">
                        ...
                        {empMenu === "sakit" && empHistory.sakit.map((h, i) => ( <tr key={i}><td className="p-4 font-black border-r bg-gray-50/50 w-24">{h.bulan}</td><td className="p-4 space-y-3 bg-gray-50/10">{h.details.map((det, idx) => ( <div key={idx} className="p-4 border rounded-2xl bg-white shadow-sm flex flex-col gap-1 border-l-4 border-l-blue-500"><div className="flex justify-between font-bold text-gray-800"><span>Libur: {det.tglTidakMasuk}</span> <span>Masuk: {det.tglMulaiMasuk}</span></div><div><span className="text-[9px] bg-blue-100 text-blue-700 font-black px-2 py-0.5 rounded uppercase">{det.keterangan}</span></div><p className="text-gray-600 pt-1"><span className="font-bold text-gray-800">Diagnosa:</span> {det.diagnosa}</p><p className="text-[10px] text-gray-400 italic">Klinik: {det.klinik}</p></div> ))}</td></tr> ))}
                        {empMenu === "sp" && empHistory.sp.map((h, i) => ( <tr key={i}><td className="p-4 font-black border-r bg-gray-50/50 w-24">{h.bulan}</td><td className="p-4 space-y-3 bg-gray-50/10">{h.details.map((det, idx) => ( <div key={idx} className="p-4 border rounded-2xl bg-white shadow-sm flex flex-col gap-1 border-l-4 border-l-orange-500"><div className="flex justify-between items-center font-bold text-gray-800"><span>Tgl Kasus: {det.tanggal}</span> <span className="text-[9px] bg-orange-100 text-orange-700 font-black px-2 py-0.5 rounded uppercase">{det.surat}</span></div><p className="text-gray-600 pt-1"><span className="font-bold text-gray-800">Pelanggaran:</span> {det.jenis}</p><p className="text-gray-600 bg-gray-50 p-2.5 rounded-lg"><span className="font-bold text-gray-500 text-[10px] block uppercase">Remarks</span> {det.remarks}</p><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1">PIC Under: {det.under}</p></div> ))}</td></tr> ))}
                      </tbody>
                    </table>
                  </div>
                </div>
