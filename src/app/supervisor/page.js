  ...
  const askAiAssistant = async (e) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        setAiChat(prev => [...prev, { role: 'assistant', text: "Maaf, AI sedang mengalami kendala: " + json.message }]);
      }
    } catch (err) {
...
allTglEmp.forEach(tgl => {
  ...
  if (!salesGroupsEmp[periode]) salesGroupsEmp[periode] = { bulan: periode, totalMemberSales: 0, totalHourlySales: 0, totalCount: 0, details: [] };
  salesGroupsEmp[periode].totalMemberSales += m.sales;
  salesGroupsEmp[periode].totalHourlySales += h.sales;
  salesGroupsEmp[periode].totalCount += h.count;
  salesGroupsEmp[periode].details.push({ tgl, memberSales: m.sales, hourlySales: h.sales, count: h.count, avgTransaction: h.count > 0 ? Math.round(h.sales / h.count) : 0 });
});
...
const overallSalesRatioEmp = totalHourlySalesEmp > 0 ? Math.round((totalMemberSalesEmp / totalHourlySalesEmp) * 1000) / 10 : null;
  ...
  const sortedData = (() => {
    ...
    arr.sort((a, b) => {
      ...
      const bothNumeric = rawA !== null && rawA !== undefined && rawA !== "" &&
                  ...
                  <div className="overflow-x-auto max-h-[60vh] overflow-y-auto">
                    <table className="w-full text-left text-xs min-w-[500px]">
                      <thead className="bg-white/80 text-[#e20074] font-black border-b sticky top-0 uppercase tracking-wider text-[9px] z-10 backdrop-blur-md">
                        {activePanel === "shortage" && (<tr><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('periode')}>Periode{sortIndicator('periode')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('nama')}>Nama Kasir{sortIndicator('nama')}</th><th className="p-4 text-center cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('frekuensi')}>Frekuensi{sortIndicator('frekuensi')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('totalShort')}>Total Short{sortIndicator('totalShort')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('totalOver')}>Total Over{sortIndicator('totalOver')}</th></tr>)}
                        {activePanel === "ecobag" && (<tr><th className="p-4">Bulan</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('staff_name')}>Nama Kasir{sortIndicator('staff_name')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('bag_la')}>Large{sortIndicator('bag_la')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('bag_me')}>Medium{sortIndicator('bag_me')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('bag_sm')}>Small{sortIndicator('bag_sm')}</th><th className="p-4 text-right font-black cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('total')}>Total{sortIndicator('total')}</th></tr>)}
                        {/* Kolom "Bulan" di Ecobag sengaja tidak diklik: sheet-nya kadang pakai kolom year_month, kadang month, jadi tidak konsisten untuk di-sort */}
                        {activePanel === "member" && (<tr><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('nama')}>Nama Kasir{sortIndicator('nama')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('bulan')}>Bulan{sortIndicator('bulan')}</th><th className="p-4 text-right font-black cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('total')}>Total Member{sortIndicator('total')}</th></tr>)}
                        {activePanel === "sakit" && (<tr><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('nama')}>Nama{sortIndicator('nama')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('tgl_tidak_masuk')}>Mulai Absen{sortIndicator('tgl_tidak_masuk')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('tgl_mulai_masuk')}>Masuk Kembali{sortIndicator('tgl_mulai_masuk')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('bulan')}>Bulan{sortIndicator('bulan')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('keterangan')}>Keterangan{sortIndicator('keterangan')}</th><th className="p-4">Diagnosa Dokter</th></tr>)}
                        {activePanel === "sales" && (<tr><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('nama')}>Nama Kasir{sortIndicator('nama')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('periode')}>Periode{sortIndicator('periode')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('totalMemberSales')}>Sales Member{sortIndicator('totalMemberSales')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('totalHourlySales')}>Sales Hourly{sortIndicator('totalHourlySales')}</th><th className="p-4 text-center cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('totalCount')}>Transaksi{sortIndicator('totalCount')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('avgTransaction')}>Rata-rata/Tx{sortIndicator('avgTransaction')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('selisih')}>Selisih{sortIndicator('selisih')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('ratio')}>% Member{sortIndicator('ratio')}</th></tr>)}
                        {activePanel === "sp" && (<tr><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('tanggal')}>Tanggal{sortIndicator('tanggal')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('nama')}>Nama Karyawan{sortIndicator('nama')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('surat_pernyataan')}>Jenis Surat{sortIndicator('surat_pernyataan')}</th><th className="p-4">Kasus / Pelanggaran</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('bulan')}>Bulan{sortIndicator('bulan')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('pic_under')}>PIC Under{sortIndicator('pic_under')}</th></tr>)}
                        {activePanel === "pwp" && (<tr><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('nama')}>Nama Kasir{sortIndicator('nama')}</th><th className="p-4 cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('periode')}>Periode{sortIndicator('periode')}</th><th className="p-4 text-right cursor-pointer select-none hover:text-pink-800" onClick={() => handleSort('totalQty')}>Total Qty{sortIndicator('totalQty')}</th></tr>)}
  ...
  <tr key={i} onClick={() => setActiveModalData({ type: 'global_sales', data: r })} className="hover:bg-indigo-50/50 transition-colors cursor-pointer group">
    ...
    <td className="p-4 text-center font-bold text-gray-500">{(r.totalCount || 0).toLocaleString("id-ID")}</td>
    <td className="p-4 text-right font-bold text-green-600">{(r.avgTransaction || 0).toLocaleString("id-ID")}</td>
...
<td className="p-4 font-bold">{r.year_month || r.month}</td><td className="p-4 font-black">{resolveNama(r.staff_name)}</td><td className="p-4 text-right text-red-500 font-bold">{r.bag_la}</td><td className="p-4 text-right text-orange-500 font-bold">{r.bag_me}</td><td className="p-4 text-right text-blue-500 font-bold">{r.bag_sm}</td><td className="p-4 text-right text-[#e20074] font-black">{r.total} Pcs</td></tr>))}
                        {activePanel === "member" && sortedData.map((r, i) => (<tr key={i} onClick={() => { const det = rawMember.filter(x => normName(x.nama) === normName(r.nama) && x.bulan === r.bulan); const details = det.map(x => ({ tgl: x.tanggal, qty: x.qty })); setActiveModalData({ type: 'global_member', data: { nama: r.nama, bulan: r.bulan, totalPerBulan: r.total, details } }); }} className="hover:bg-pink-50/50 transition-colors cursor-pointer"><td className="p-4 font-black">{r.nama}</td><td className="p-4 font-bold">{r.bulan}</td><td className="p-4 text-right font-black text-lg text-[#e20074]">{r.total}</td></tr>))}
                        {activePanel === "sakit" && sortedData.map((r, i) => (<tr key={i} className="hover:bg-pink-50/50 transition-colors"><td className="p-4 font-black">{resolveNama(r.nama)}</td><td className="p-4 font-bold">{r.tgl_tidak_masuk}</td><td className="p-4 font-bold">{r.tgl_mulai_masuk}</td><td className="p-4 text-gray-400 font-bold">{r.bulan}</td><td className="p-4"><span className="bg-blue-50 text-blue-600 font-black px-2.5 py-1 rounded-md text-[9px] uppercase">{r.keterangan}</span></td><td className="p-4 font-bold text-gray-600">{r.reason_diagnosa} <p className="text-[10px] italic font-normal text-gray-400 mt-0.5">{r.alamat_klinik}</p></td></tr>))}
                        {activePanel === "sp" && sortedData.map((r, i) => (<tr key={i} className="hover:bg-pink-50/50 transition-colors"><td className="p-4 font-bold">{r.tanggal}</td><td className="p-4 font-black">{resolveNama(r.nama)}</td><td className="p-4"><span className="bg-orange-50 text-orange-600 font-black px-2.5 py-1 rounded-md text-[9px] uppercase">{r.surat_pernyataan}</span></td><td className="p-4"><p className="font-bold text-gray-800">{r.jenis_pelanggaran}</p><p className="text-[10px] text-gray-500 font-medium mt-0.5">{r.remarks}</p></td><td className="p-4 text-gray-400 font-bold">{r.bulan}</td><td className="p-4 text-gray-500 font-black">{r.pic_under}</td></tr>))}
                  ...
                  <div className="overflow-x-auto max-h-[50vh] overflow-y-auto">
                    <table className="w-full text-left text-xs min-w-[400px]">
                      <thead className="bg-white/80 text-[#e20074] font-black border-b uppercase tracking-wider text-[9px] sticky top-0 z-10 backdrop-blur-md">
                        ...
                        {empMenu === "sales" && (<tr><th className="p-4">Periode</th><th className="p-4 text-right">Sales Member</th><th className="p-4 text-right">Sales Hourly</th><th className="p-4 text-center">Transaksi</th><th className="p-4 text-right">Rata-rata/Tx</th><th className="p-4 text-right">Selisih</th><th className="p-4 text-right">% Member</th></tr>)}
  ...
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-6">
    <div className="bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl anim-pop-in">
      ...
      <div className="p-4 bg-gray-50 border-b grid grid-cols-2 gap-2">
        ...
        <div className="bg-green-50 p-3 rounded-2xl text-center border border-green-100"><p className="text-[9px] text-green-500 font-extrabold uppercase">Rata-rata/Tx</p><p className="font-black text-sm text-green-600 mt-1">{(activeModalData.data.avgTransaction || 0).toLocaleString('id-ID')}</p></div>
      ...
      <div className="p-5 max-h-[50vh] overflow-y-auto space-y-3 bg-gray-50/50">
        {activeModalData.data.details.map((det, i) => {
          ...
          <div key={i} className="p-4 border border-gray-200 rounded-2xl bg-white shadow-sm text-[11px] space-y-1 border-l-[5px] border-l-indigo-400 hover:shadow-md transition-shadow">
            ...
            <div className="flex justify-between border-t pt-1.5 mt-1"><span className="text-gray-500 font-bold uppercase tracking-wider text-[9px]">% Kontribusi Member:</span><span className="font-black text-purple-600">{detRatio}%</span></div>
          </div>
          );
        })}
      </div>
