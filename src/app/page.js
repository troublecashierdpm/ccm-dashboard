...
export default function App() {
  ...
  const fetchDashboardData = async () => {
    try {
      // --- FUNGSI BARU: PENARIK DATA BERANTAI (ANTI LIMIT 1000 BARIS SUPABASE) ---
      const fetchUserRecords = async (table, column) => {
        ...
        while (true) {
          ...
          if (error) {
            console.error(`Error fetch ${table}:`, error);
            break;
          }
          if (data && data.length > 0) {
            all = [...all, ...data];
            if (data.length < step) break; // Berhenti jika data yang ditarik kurang dari 1000 (sudah habis)
            from += step;
          } else {
            break;
          }
