...
export default function App() {
    ...
    try {
      ...
      const finalMemberHistory = Object.values(memberGroups)
        ...
        .map(group => ({
          ...
          details: Object.keys(group.dailyMap)
            .map(tgl => ({ tgl, qty: group.dailyMap[tgl] }))
            .sort((a,b) => (parseInt(b.tgl.split('-')[0])||0) - (parseInt(a.tgl.split('-')[0])||0))
        }));
  ...
  const prosesLogin = async () => {
    ...
    if (error) { 
      // Catat ke log_login sebagai FAILED
    ...
    // Catat ke log_login sebagai SUCCESS
